...
local minv = { { 0.41239079926595, 0.35758433938387, 0.18048078840183 },
  { 0.21263900587151, 0.71516867876775, 0.072192315360733 }, { 0.019330818715591, 0.11919477979462, 0.95053215224966 } }
...
local function xyz__3ergb(tuple)
  return { from_linear(dot_product(m[1], tuple)), from_linear(dot_product(m[2], tuple)),
...
local function random_color(red_range, green_range, blue_range)
  local rgb = { b = math.random(blue_range[1], blue_range[2]), r = math.random(red_range[1], red_range[2]),
...
return { ["blend-hex"] = blend_hex, ["lighten-hex"] = lighten_hex, ["darken-hex"] = darken_hex,
  ["saturate-hex"] = saturate_hex, ["desaturate-hex"] = desaturate_hex, ["rotate-hex"] = rotate_hex, gradient = gradient,
  ["gradient-n"] = gradient_n, ["generate-pallete"] = generate_pallete, ["to_linear"] = to_linear }
